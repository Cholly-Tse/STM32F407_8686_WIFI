...
extern struct lbs_private *pgmarvel_priv;//改下变量的位置
...
////  if_sdio_handle_cmd(card, card->buffer + 4, chunk - 4)
		...
		u8 *buffer, unsigned size)
{
 ...
 ///? chuk 是什么
	if (size > LBS_CMD_BUFFER_SIZE) {
	   // printf("size > LBS_CMD_BUFFER_SIZE:%d",size);
	   // printf("response packet too large   = \n   ",(int)size);
...
static int if_sdio_card_to_host(struct if_sdio_card *card)
{
	...
	struct lbs_private	*priv=card->priv;
//	printf("enter if_sdio_card_to_host\n");
	//lbs_deb_enter(LBS_DEB_SDIO);
...
out:
   ...
   // printf("left if_sdio_card_to_host\n");
	return ret;
}
...
static int if_sdio_prog_helper(struct if_sdio_card *card)
{
	...
	const u8 *firmware;
	u32 size;
//	printf("enter if_sdio_prog_helper\n");
...
static int if_sdio_prog_real(struct if_sdio_card *card)
{
	...
	while (size) {
		...
		//	printf("sending %d bytes (%d bytes) chunk\n",
	...
	ret = 0;
	printf("waiting for firmware to boot...\n");
