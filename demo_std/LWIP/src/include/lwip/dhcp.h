...
#define DHCP_OPTION_REQUESTED_IP_SIZE 4
...
#ifndef DHCP_ADDR_START //now the max number of hosts is 255
#define  DHCP_ADDR_START 100
#endif

#ifndef DHCP_ADDR_END 
#define  DHCP_ADDR_END 200
#endif

#define BIT_MAP_SIZE ((DHCP_ADDR_END-DHCP_ADDR_START)/sizeof(u8_t)+1)

struct dhcp_client{
	u8_t hwaddr[DHCP_CHADDR_LEN];//hardware mac addr
	u8_t name[DHCP_SNAME_LEN];
	struct ip_addr ipaddr;
	//u8_t state;//current client state 
...
struct dhcp_srv{
  /** current DHCP state machine state */
  u8_t bitmap[BIT_MAP_SIZE];//dhcp client pool
