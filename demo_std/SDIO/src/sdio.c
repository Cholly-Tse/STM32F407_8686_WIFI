...
int mmc_send_io_op_cond(struct mmc_host *host, u32 ocr, u32 *rocr)
{
	...
	for (i = 100; i; i--) {
		err = mmc_wait_for_cmd(host, &cmd, MMC_CMD_RETRIES);   //等待回应吗？
		if (err)
			break;
...
/*
	...
	if (mmc_card_highspeed(card)) {
		 ...
		 * mandatory.
		 */
// 		mmc_set_clock(host,72000000);//2015xxx20150724
		mmc_set_clock(host,48000000);//2015xxx0820
	} else {
...
int mmc_attach_sdio(struct mmc_host *host, u32 ocr)
{
	 ...
	 */
	if (ocr & 0x7F) {
		printf(KERN_WARNING "%s: card claims to support voltages "
	 ...
	 */
//	 printf("device function count=%d", funcs);
	for (i = 0;i < funcs;i++) {
		err = sdio_init_func(host->card, i + 1);
		if (err)
			goto remove;
	}
//	printf("device function inital is ok!\n");
...
remove_added:
	printf("Remove without lock if the device has been added.\n");
remove:
	printf("And with lock if it hasn't been added.\n");	
err:
	printf(KERN_ERR "%s: error %d whilst initialising SDIO card\n",
...
static int process_sdio_pending_irqs(struct mmc_card *card)
{
	...
	if (ret) {
		printf(KERN_DEBUG "error %d reading SDIO_CCCR_INTx\n",ret);
		return ret;
	}
	...
	for (i = 1; i <= 7; i++) {
		if (pending & (1 << i)) {
			...
			if (!func) {
			//	printf(KERN_WARNING "pending IRQ for "
			...
			//	printf(KERN_WARNING "pending IRQ with no handler\n");
				ret = -EINVAL;
			}
		}
	}
...
//	printf("%s: IRQ thread exiting with code %d\n",
